...
<form id="productForm" method="POST" action="{{ route('admin.products.store') }}" enctype="multipart/form-data">
  ...
  <div class="add-product-grid" style="display:grid;grid-template-columns:2fr 1fr;gap:16px;align-items:start;">
    ...
    <div>
      ...
      <div class="card mb-16">
        <div class="card-body">
          <div class="form-section-title">🖼️ Ảnh sản phẩm phụ <span style="font-size:.72rem;color:var(--text-3);font-weight:400;">(tối đa 5 ảnh, mỗi ảnh ≤ 5MB)</span></div>
          <div id="sub-images-upload-area" style="position:relative;border:2px dashed var(--border);border-radius:var(--r-md);padding:16px;text-align:center;cursor:pointer;transition:all .2s;background:var(--bg);" onmouseenter="this.style.borderColor='var(--g-light)'" onmouseleave="this.style.borderColor='var(--border)'">
            ...
            <p><strong>Nhấp để chọn ảnh phụ</strong></p>
        ...
        <div class="card-body">
          ...
          <div style="display:flex;gap:0;border:1.5px solid var(--border);border-radius:var(--r-md);overflow:hidden;margin-bottom:14px;">
            <button type="button" id="tab-upload-btn" onclick="switchVideoTab('upload')"
              style="flex:1;padding:9px;border:none;font-size:.8rem;font-weight:600;cursor:pointer;background:var(--g-dark);color:#fff;transition:all .2s;">
              📁 Upload file
            </button>
            <button type="button" id="tab-youtube-btn" onclick="switchVideoTab('youtube')"
              style="flex:1;padding:9px;border:none;font-size:.8rem;font-weight:600;cursor:pointer;background:var(--bg);color:var(--text-2);transition:all .2s;">
              ▶ YouTube / Shorts
            </button>
          </div>
    ...
    <div>
      ...
      <div class="card mb-16">
        <div class="card-body">
          ...
          <div id="main-img-upload-area" style="position:relative;border:2px dashed var(--border);border-radius:var(--r-md);padding:20px;text-align:center;cursor:pointer;transition:all .2s;background:var(--bg);" onmouseenter="this.style.borderColor='var(--g-light)'" onmouseleave="this.style.borderColor='var(--border)'">
            ...
            <p><strong>Nhấp để tải ảnh chính</strong></p>
...
<script>
  ...
  function previewSubImages(event) {
    ...
    const allowed = 5 - currentNewCount;
  ...
  function switchVideoTab(tab) {
    ...
    if (tab === 'upload') {
      uploadBtn.style.background = 'var(--g-dark)';  uploadBtn.style.color = '#fff';
      ytBtn.style.background = 'var(--bg)';           ytBtn.style.color = 'var(--text-2)';
      panelUp.style.display = 'block';
      panelYt.style.display = 'none';
      document.getElementById('videoSourceInput').value = 'upload';
    } else {
      ytBtn.style.background = 'var(--g-dark)';     ytBtn.style.color = '#fff';
      uploadBtn.style.background = 'var(--bg)';      uploadBtn.style.color = 'var(--text-2)';
